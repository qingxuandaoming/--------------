...
或使用脚本：`pwsh ./scripts/setup_venv.ps1`。

## 依赖管理与工具

- 使用 `requirements.txt` 固定依赖版本，避免冲突。
- 可选使用 `pyproject.toml` 安装本包：`pip install .`。
- 建议在虚拟环境中进行开发与测试；不在全局环境安装 PyAudio。
...
- 输出数据（`.npz`）默认保存到仓库根目录的 `data/`，可通过环境变量覆盖：`RTP_SAVE_DIRECTORY=./my_outputs`。
- 版本库已忽略 `data/` 目录与 `voice_processing_data_*.npz` 文件；使用 `make clean` 可一键清理输出与文档构建产物。

### UI 模块化调整

- 为减少单文件体积与提升可维护性，界面样式与文件工具已拆分：
  - `real_time_voice_processing/ui/styles.py`：提供 `DEFAULT_PALETTE` 与 `build_stylesheet`。
  - `real_time_voice_processing/ui/file_utils.py`：提供 `default_audio_dir` 与 `collect_audio_files`。
- `visualization.py` 已更新引用，功能与视觉效果保持一致。
...
- Python 命名使用 `snake_case`（例如：`audio_source.py`）。
- 注释与文档字符串使用 NumPy 风格（`Parameters`/`Returns`/`Examples`）。
- Markdown 文档遵循规范：
  - 代码块必须指定语言（避免 MD040），如 ` ```python `、` ```bash `。
  - 列表前后保留空行（避免 MD032）。
- 数学公式使用 LaTeX 语法（本项目已启用 MathJax 与 MyST），例如：`$E=mc^2$`。

## 提交流程建议

- 在本地通过 `pytest -q` 运行单元测试；确保新增/修改代码有相应测试。
- 更新相关文档与示例，保持 README 与 Sphinx 文档一致。
- 变更涉及 UI 时，请附带截图或说明，并在文档中标注更新内容。
