...
- 阈值：`ENERGY_THRESHOLD=1000`、`ZCR_THRESHOLD=0.25`
...
- 独立显示缓冲：`audio_display_buffer` 与处理缓冲分离，避免波形因被消费而不连续。
- 自适应阈值校准：启动后使用前 `CALIBRATION_FRAMES` 帧分位数估计能量、ZCR 与谱熵阈值。
- 文件实时模拟：离线文件按墙钟节流喂给引擎（`SIMULATE_REALTIME_FILES`），并支持同步播放以便比对。
...
│   └── audio_source.py      # 输入源抽象（麦克风/文件/播放列表）
├── ui/
│   ├── visualization.py     # 可视化界面与交互控件
│   ├── styles.py            # UI 样式工具
│   └── file_utils.py        # 音频文件工具
├── tests/
...
│   └── 常见问题.md                # FAQ 与故障排除
├── README.md                # 项目说明文档
└── requirements.txt         # 依赖包列表（包内）
```

说明：仓库根目录提供统一的 `Makefile` 构建入口（跨平台），用于创建虚拟环境、安装依赖、运行程序与测试、构建文档等。
