...
```python
def voice_activity_detection(energy, zcr, energy_threshold=1000, zcr_threshold=0.25):
    """语音活动检测（固定阈值）：高能量 + 低 ZCR 判为语音"""
    # 第一级：能量检测（过滤低能量）
    if energy < energy_threshold:
        return 0

    # 第二级：过零率检测（清音通常 ZCR 较低）
...
1. **第一级判决**：使用较高的能量阈值 $T_E$ 进行粗判，高于 $T_E$ 的更可能是语音
2. **第二级判决**：使用过零率阈值 $T_Z$ 进行细判，低于 $T_Z$ 判为语音
